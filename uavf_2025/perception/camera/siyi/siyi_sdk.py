...
class SIYISDK:
    """
    API for interfacing with pan/tilt/zoom on SIYI brand camera gimbals
    """
    ...
    def setAbsolutePosition(self, yaw, pitch):
        """
    ...
    def getFirmwareVersion(self):
        return (
            self._fw_msg.code_board_ver,
            self._fw_msg.gimbal_firmware_ver,
            self._fw_msg.zoom_firmware_ver,
        )
    ...
    def _manual_absolute_zoom(self, target: float, timeout=15) -> bool:
        ...
        while self.getZoomLevel() != target:
            ...
            if time() - temp > timeout:
                self._logger.error(
                    "Absolute Zoom Timeout at zoom level: %.1f", self.getZoomLevel()
                )
                self.requestZoomHold()
                return False
