...
@dataclass
class CameraPose:
    # TODO: Refactor to use Pose class
    position: np.ndarray
    rotation: Rotation
    focal_len_px: float

    def with_pose(self, pose: Pose) -> CameraPose:
        """
        Returns a new CameraPose of the camera in the global frame, if the camera is attached to an object with the given pose.
        Use this to combine the camera and drone pose to get a camera pose in the global frame.
        """
        return CameraPose(
            pose.position + pose.rotation.apply(self.position),
...
class Bbox2D(NamedTuple):
    ...
    @staticmethod
    def from_xyxy(xyxy: list[float]) -> Bbox2D:
        x1, y1, x2, y2 = xyxy
        return Bbox2D(
            (x1 + x2) / 2,
            (y1 + y2) / 2,
...
class TargetBBox:
    bbox: Bbox2D
    # cls_probs is basically a list[float]
    cls_probs: np.ndarray  # Usually almost a one-hot vector, ex: [0, 0, 0, 0.45, 0]
    img_id: int
    det_id: int
...
class Image(Generic[_UnderlyingImageT]):
    ...
    def make_tile(self, x_coord, y_coord, tile_size) -> Tile:
        ...
        """
        return Tile(
            self.make_sub_image(x_coord, y_coord, tile_size, tile_size),
            x_coord,
            y_coord,
        )
    ...
    def generate_tiles(
        self, tile_size: int, min_overlap: int = 0
        ...
        x_count = np.uint8(
            np.ceil((img_width - min_overlap) / (tile_size - min_overlap))
        )
        y_count = np.uint8(
            np.ceil((img_height - min_overlap) / (tile_size - min_overlap))
        )
        ...
        remaindersX = np.ones((x_count - 1,), dtype=np.uint8) * img_coord_t(
            np.floor(overflow_x / (x_count - 1))
        )
        remaindersY = np.ones((y_count - 1,), dtype=np.uint8) * img_coord_t(
            np.floor(overflow_y / (y_count - 1))
        )
        ...
        for vertical_index in range(y_count):
            ...
            for horizontal_index in range(x_count):
                ...
                if horizontal_index < (x_count - 1):
                    next_horizontal_overlap = (
                        min_overlap + remaindersX[horizontal_index]
                    )
