...
import rclpy
import rclpy.node
from rclpy.qos import HistoryPolicy, DurabilityPolicy, QoSProfile, ReliabilityPolicy
        ...
        if crc8(buff[:-1]) == buff[-1]:
            ...
            if len(last_ten) == 21:
                ...
                if statustext_timer == 0:
                    for chunk in [
                        status_str[i : i + 30] for i in range(0, len(status_str), 30)
                    ]:
                        node.msg_pub.publish(
                            mavros_msgs.msg.StatusText(
                                severity=mavros_msgs.msg.StatusText.NOTICE, text=chunk
                            )
                        )
                    statustext_timer = STATUSTEXT_PERIOD
